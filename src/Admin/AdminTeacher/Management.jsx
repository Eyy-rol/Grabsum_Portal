...
/**
 * IMPORTANT NOTES (based on your schemas):
 * - teachers.user_id is PK and FK -> profiles.user_id -> auth.users.id
 * - employee_number is generated by Edge Function (NOT by frontend)
 * - auth "username" will be employee_number, but actual auth login uses a generated email (internal)
 * - real email (@gmail.com) is stored in teachers.email and profiles.email (for reference)
 * - "Delete" in UI should archive (soft delete): is_archived = true (teachers + profiles)
 */

const teacherSchema = z.object({
  // employee_number is generated server-side; keep it in form so it can be displayed (disabled)
...
export default function Management() {
  ...
  // for showing credentials after creation/reset
  ...
  const createM = useMutation({
    mutationFn: async (values) => {
      ...
      // - create auth user (internal/fake email)
      // - insert into profiles (role teacher, must_change_password true, timestamps)
      // - insert into teachers
      const { data, error } = await supabase.functions.invoke("create-teacher", {
        body: {
          first_name: values.first_name,
          last_name: values.last_name,
          email: values.email, // real email (reference)
  ...
  // UPDATE (teachers table only; profile email syncing handled by your trigger or keep same)
  ...
  // Admin Reset Password via Edge Function: admin-reset-teacher-password
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <div className="text-xs text-black/55">
            Add, view, update, archive, restore, and reset teacher passwords.
          </div>
        </div>
...
function TeacherModal({ mode, row, onClose, onCreate, onUpdate, busy }) {
  ...
  const defaults = useMemo(() => {
    const base = {
      employee_number: "", // generated after create
      department: "",
      position: "",
      status: "Active",
      first_name: "",
        ...
        <form onSubmit={handleSubmit(submit)} className="h-[calc(100%-64px)] overflow-auto p-4 space-y-4">
          <div className="rounded-2xl border border-black/10 bg-white/70 p-4">
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              ...
              <Input label="First Name *" disabled={isView} error={errors.first_name?.message} {...register("first_name")} />
              <Input label="Last Name *" disabled={isView} error={errors.last_name?.message} {...register("last_name")} />

              <Input label="Email (real) *" disabled={isView} error={errors.email?.message} {...register("email")} />
              <Input label="Contact Number" disabled={isView} error={errors.contact_number?.message} {...register("contact_number")} />
